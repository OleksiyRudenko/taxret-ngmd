/*
 * ng-telephone
 *
 * based on:
 * ng-currency
 * http://alaguirre.com/
 * Version: 0.9.6 - 2016-04-04
 * License: MIT
 */

(function(){

  /*commonjs support*/
  if (typeof module !== "undefined" && typeof exports !== "undefined" && module.exports === exports){
    module.exports = 'ng-telephone';
  }

  /**
   * @desc Directive that converts +000000000 model into formatted human-readable view. uses telephone* filters
   * @example <input data-ng-telephone ng-model="telNumber" telephone-country="UA" />
   * @example <input data-ng-telephone ng-model="telNumber" /> -- autodetect country for formatting from tel nr
   *          NB! Currently auto-detect brute-forcely assigned UA.
   * @example <input data-ng-telephone="false" ng-model="telNumber" /> -- disable formatter
   */

  angular.module('ng-telephone', [])
    .directive('ngTelephone', ngTelephone);

  ngTelephone.$inject=['$filter'];

  function ngTelephone($filter) {
    var directive = {
      require: 'ngModel',
      scope: {
        minlength: '=?minlength',
        maxlength: '=?maxlength',
        telephoneCountry: '=?telephoneCountry',
        ngRequired: '=?ngRequired',
        // fraction: '=?fraction',
      },
      link: link,
    };

    return directive;

    function link(scope, element, attrs, ngModel) {
        if (attrs.ngTelephone === 'false') return;

        scope.telephoneCountry = (typeof scope.telephoneCountry !== 'undefined')
          ? scope.telephoneCountry
          : 'auto';

        /*
        function decimalRex(dChar) { //?
          return RegExp("\\d|\\-|\\" + dChar, 'g');
        }

        function clearRex(dChar) { //?
          return RegExp("\\-{0,1}((\\" + dChar + ")|([0-9]{1,}\\" + dChar + "?))&?[0-9]{0," + scope.fraction + "}", 'g');
        }

        function clearValue(value) { //?
            value = String(value);
            var dSeparator = $locale.NUMBER_FORMATS.DECIMAL_SEP;
            var cleared = null;

            if(value.indexOf($locale.NUMBER_FORMATS.DECIMAL_SEP) == -1 &&
              value.indexOf('.') != -1 &&
              scope.fraction)
            {
              dSeparator = '.';
            }

            // Replace negative pattern to minus sign (-)
            var neg_dummy = $filter('currency')("-1", getCurrencySymbol(), scope.fraction);
            var neg_regexp = RegExp("[0-9."+$locale.NUMBER_FORMATS.DECIMAL_SEP+$locale.NUMBER_FORMATS.GROUP_SEP+"]+");
            var neg_dummy_txt = neg_dummy.replace(neg_regexp.exec(neg_dummy), "");
            var value_dummy_txt = value.replace(neg_regexp.exec(value), "");

            // If is negative
            if(neg_dummy_txt == value_dummy_txt) {
              value = '-' + neg_regexp.exec(value);
            }

            if(RegExp("^-[\\s]*$", 'g').test(value)) {
              value = "-0";
            }

            if(decimalRex(dSeparator).test(value))
            {
              cleared = value.match(decimalRex(dSeparator))
                .join("").match(clearRex(dSeparator));
              cleared = cleared ? cleared[0].replace(dSeparator, ".") : null;
            }

            return cleared;
        }

        function getCurrencySymbol() { //?
          if (angular.isDefined(scope.currencySymbol)) {
            return scope.currencySymbol;
          } else {
            return $locale.NUMBER_FORMATS.CURRENCY_SYM;
          }
        } */

        function reformatViewValue(){
          var formatters = ngModel.$formatters,
            idx = formatters.length;

          var viewValue = ngModel.$$rawModelValue;
          while (idx--) {
            viewValue = formatters[idx](viewValue);
          }

          ngModel.$setViewValue(viewValue);
          ngModel.$render();
        }

        ngModel.$parsers.push(function (viewValue) {
          return $filter('telephoneSanitize')(viewValue, scope.telephoneCountry);
          /* var cVal = clearValue(viewValue);
          //return parseFloat(cVal);
          // Check for fast digitation (-. or .)
          if(cVal == "." || cVal == "-.")
          {
            cVal = ".0";
          }
          return parseFloat(cVal);
          */
        });

        element.on("blur", function () {
          ngModel.$commitViewValue();
          reformatViewValue();
        });

        ngModel.$formatters
          .unshift(function (value) {
            return $filter('telephone')(value, scope.telephoneCountry);
          });

        ngModel.$validators.minlength = function(cVal) {
          if (!scope.ngRequired && isNaN(cVal)) {
            return true;
          }
          if(typeof scope.minlength  !== 'undefined') {
            return cVal.length >= parseInt(scope.minlength);
          }
          return true;
        };

        scope.$watch('minlength', function (val) {
          ngModel.$validate();
        });

        ngModel.$validators.maxlength = function(cVal) {
          if (!scope.ngRequired && isNaN(cVal)) {
            return true;
          }
          if(typeof scope.maxlength  !== 'undefined') {
            return cVal.length <= parseInt(scope.maxlength);
          }
          return true;
        };

        scope.$watch('maxlength', function (val) {
          ngModel.$validate();
        });

        /*
        ngModel.$validators.fraction = function(cVal) {
          if (!!cVal && isNaN(cVal)) {
            return false;
          }

          return true;
        };
        */

        // One can redraw all directives broadcasting "telephoneRedraw" event.
        scope.$on('telephoneRedraw', function() {
          ngModel.$commitViewValue();
          reformatViewValue();
        });

        element.on('focus',function() {
          var viewValue = ngModel.$$rawModelValue;

          if(isNaN(viewValue) || viewValue === '' || viewValue == null)
          {
            viewValue = '';
          }
          /* // do not change otherwise
          else
          {
            viewValue = parseFloat(viewValue).toFixed(scope.fraction);
          } */
          ngModel.$setViewValue(viewValue);
          ngModel.$render();
        });
      }
    }

})();